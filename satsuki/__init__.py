...
class Arguments:
    """
    Configuration information needed by the satsuki.ReleaseMgr class.
    ...
    def __init__(self, **kwargs):
        ...
        if self.opts["internal_cmd"] == Arguments.INTERNAL_CMD_CREATE and (
            ...
            raise_error("Must include tag when creating release", AttributeError)
        ...
        if self.opts["internal_cmd"] == Arguments.INTERNAL_CMD_CREATE:
            ...
            or self.opts["internal_cmd"] == Arguments.INTERNAL_CMD_RECREATE
        ):
    ...
    def _init_basic(self, kwargs):
        ...
        if self.opts["api_token"] is None:
            raise_error("No GitHub API token was provided.", PermissionError)

        # user command
        if not kwargs.get("command", False):
            self.opts["user_cmd"] = Arguments.CMD_UPSERT
        elif (
            kwargs.get("command") == Arguments.CMD_UPSERT
            or kwargs.get("command") == Arguments.CMD_DELETE
        ):
            self.opts["user_cmd"] = kwargs.get("command")
        else:
            raise_error("Invalid command:" + kwargs.get("command"), AttributeError)
        ...
        self.opts["slug"] = kwargs.get(
            ...
            os.environ.get(
                "TRAVIS_REPO_SLUG",
                os.environ.get(
                    "APPVEYOR_REPO_NAME", os.environ.get("BUILD_REPOSITORY_NAME", None)
                ),
            ),
        )
        ...
        if self.opts["slug"] is None and (
            ...
            self.opts["slug"] = "/".join([self.opts["user"], self.opts["repo_name"]])
        ...
        self.opts["target_commitish"] = kwargs.get(
            ...
            os.environ.get(
                "TRAVIS_COMMIT",
                os.environ.get(
                    "APPVEYOR_REPO_COMMIT", os.environ.get("BUILD_SOURCEVERSION", None)
                ),
            ),
        )

        self.opts["gb_info_file"] = kwargs.get(
            "gb_info_file", os.environ.get("GB_INFO_FILE", Arguments.GB_INFO_FILE)
        )

    def _init_gb_info(self):
        """Get GB (GravityBee) info, if any."""
        gb_info = None
        self.gb_subs = {}
        ...
        if os.path.exists(self.opts["gb_info_file"]):
            ...
            with open(self.opts["gb_info_file"], "r", encoding="utf8") as info_file:
            ...
            if gb_info.get("app_name", None) is not None:
                ...
                self.gb_subs["gb_pkg_name_lower"] = gb_info["app_name"].lower()
    ...
    def _init_tag(self, kwargs):
        ...
        if self.opts["tag"]:
            self.opts["tag"] = Template(self.opts["tag"]).safe_substitute(self.gb_subs)
        ...
        if not isinstance(self.opts["tag"], str) and not self.flags["latest"]:
            ...
            self.opts["tag"] = os.environ.get(
                "TRAVIS_TAG", os.environ.get("APPVEYOR_REPO_TAG_NAME", None)
            )
            if self.opts["tag"] is None:
                raise_error(
                    "Either tag or the latest flag is required.", AttributeError
                )
    ...
    def _init_internal_command(self):
        ...
        if self.get_release():
            ...
            if self.opts["user_cmd"] == Arguments.CMD_UPSERT:
                ...
                if (
                    self.working_tag is not None
                    and hasattr(self.working_tag, "commit")
                    and self.opts["target_commitish"] is not None
                    and self.working_tag.commit.sha != self.opts["target_commitish"]
    ...
    def _init_files(self, kwargs):
        """
        Handle multple files from command line args or a files_file.
    ...
    def _init_files_file(self):
        ...
        if self.opts["files_file"] and os.path.isfile(self.opts["files_file"]):
            with open(self.opts["files_file"], "r", encoding="utf8") as files_file:
                self.lists["file_info"] += json.loads(files_file.read())

    def _init_gb_files_file(self):
        """Handle the GravityBee files_file."""
        if (
            os.path.exists(Arguments.GB_FILES_FILE)
            and self.opts["user_cmd"] == Arguments.CMD_UPSERT
        ):
            with open(Arguments.GB_FILES_FILE, "r", encoding="utf8") as files_file:
                self.lists["file_info"] += json.loads(files_file.read())
    ...
    def _init_cmd_line_files(self):
        ...
        if self.lists["files"]:
            logger.info("Processing command-line files: %d", len(self.lists["files"]))

            if len(self.lists["files"]) != len(self.lists["labels"]) and len(
                self.lists["labels"]
            ) not in [0, 1]:
                raise_error(
            ...
            if len(self.lists["files"]) != len(self.lists["mimes"]) and len(
                self.lists["mimes"]
            ) not in [0, 1]:
                raise_error(
                    "Invalid number of MIME types: " + len(self.lists["mimes"]),
                    AttributeError,
                )
        ...
        for i, filename in enumerate(new_files):
            ...
            if self.lists["labels"]:
                ...
                if len(self.lists["labels"]) == 1:
                    info["label"] = Template(self.lists["labels"][0]).safe_substitute(
                        self.gb_subs
                    )
                else:
                    info["label"] = Template(self.lists["labels"][i]).safe_substitute(
                        self.gb_subs
                    )
    ...
    def _init_process_files(self):
        ...
        if self.lists["file_info"] and self.opts["user_cmd"] == Arguments.CMD_UPSERT:
            ...
            for info in self.lists["file_info"]:
                ...
                if os.path.isfile(info["path"]):
                    ...
                    logger.info("Skipping file. %s does not exist", info["path"])
    ...
    def _init_check_no_files(self):
        if self.lists["files"] and not self.lists["file_info"]:
            raise_error(
                "File flag used but no matching files were found", AttributeError
            )
    ...
    def _init_data(self, kwargs):
        ...
        if self.opts["body"] is None:
            ...
            self.opts["body"] = Template(self.opts["body"]).safe_substitute(
                self.gb_subs
            )
        ...
        if self.opts["rel_name"] is None:
            ...
            # possible template expansion
            self.opts["rel_name"] = Template(self.opts["rel_name"]).safe_substitute(
                self.gb_subs
            )
    ...
    def get_release(self):
        ...
        github_conn = github.Github(self.opts["api_token"], per_page=Arguments.PER_PAGE)
...
class ReleaseMgr:
    ...
    def _create_release(self):
        ...
        if isinstance(self.args.opts["tag"], int):
            ...
            raise_error("Integer tag name given: " + self.args.opts["tag"], TypeError)
        if not isinstance(self.args.opts["target_commitish"], str):
            raise_error("Commit SHA is required", AttributeError)
    ...
    def _handle_upload_error(self, upload_error, file_info, complete_filesize):
        ...
        if isinstance(
            ...
            if (
                self._find_release_asset(file_info["filename"])
    ...
    def _upload_file(self, file_info):
        ...
        while attempts < Arguments.MAX_UPLOAD_ATTEMPTS and not success:
            ...
            logger.info(
                "Attempt: %s", str(attempts) + "/" + str(Arguments.MAX_UPLOAD_ATTEMPTS)
            )
            ...
            if (
                ...
                success = True
            elif self._handle_upload_error(upload_error, file_info, complete_filesize):
                success = True
    ...
    def _upload_files(self):
        ...
        for file_info in self.args.lists["file_info"]:
            ...
            logger.info(
                "Uploading file %s...", str(file_uploading) + "/" + str(files_to_upload)
            )
    ...
    def _delete_tag(self):
        ...
        if (
            self.args.opts["internal_cmd"] == Arguments.INTERNAL_CMD_DELETE_TAG
            ...
            for tag in self.args.repo.get_tags():
                if fnmatch.fnmatch(tag.name, self.args.opts["tag"]):
                    try:
                        ...
                        try:
                            subprocess.run(
                                ["git", "tag", "--delete", tag.name], check=True
                            )
    ...
    def execute(self):
        ...
        if self.args.opts["internal_cmd"] == Arguments.INTERNAL_CMD_DELETE_FILE:
            self._delete_file()
        elif self.args.opts["internal_cmd"] == Arguments.INTERNAL_CMD_DELETE_REL:
            self._delete_release()
            self._delete_tag()
        elif self.args.opts["internal_cmd"] == Arguments.INTERNAL_CMD_DELETE_TAG:
            self._delete_tag()
