...
def test_sha_hash():
    ...
    assert (
        Arguments.get_hash(os.path.join("tests", "sha_hash_test.txt"))
...
def test_get_repo(mock_get_repo):
    ...
    Arguments(token="abc", slug=TEST_SLUG, tag=TEST_TAG)
...
def test_get_latest_repo(mock_get_repo):
    ...
    args = Arguments(
        token="abc", slug="bad slug", latest=True, user="user", repo="repo"
    )
...
def test_no_release_no_tag(mock_get_repo):
    ...
    mock_get_repo.return_value.get_latest_release.side_effect = github.GithubException(
        "status", "data", None
    )

    with pytest.raises(AttributeError):
        Arguments(token="abc", slug=TEST_SLUG, latest=True)
...
def test_no_release(mock_get_repo):
    ...
    # this is to simulate no repo existing yet, i.e., need to create
    mock_get_repo.return_value.get_release.side_effect = github.GithubException(
        "status", "data", None
    )

    args = Arguments(token="abc", slug=TEST_SLUG, tag=TEST_TAG)
...
def test_create_execute(mock_get_repo):
    ...
    args = Arguments(
        token="abc", slug=TEST_SLUG, tag=TEST_TAG, commitish=TEST_COMMITISH
    )
